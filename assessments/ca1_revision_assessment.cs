 ...
 *                  (ii)Produce a report  on the people looking for quotes, giving a breakdown of the  numbers of customers in each age category, and calculating the average quote given for each.
...
namespace year1_sem2_CA1
{
    class Program
    {
        ...
        {                                                            
            ...
            string[] customerDeatils = new string[4];               // define array to store the customers details in
            int length = customerDeatils.Length;                    // calc. length of array to use in for loop
            
            Console.Write(INPUTTAB, "Enter Vehicle value", ": ");
            customerDeatils[0] = Console.ReadLine();
            Console.Write(INPUTTAB, "Enter age", ": ");
            int age = Convert.ToInt32(Console.ReadLine());
            if (age  > 18)
            {
                Console.Write(INPUTTAB, "Enter Years No Claims Bonus", ": ");
                customerDeatils[2] = Console.ReadLine();
                Console.WriteLine(INPUTTAB, "Enter Penalty points", ": ");
                customerDeatils[3] = Console.ReadLine();
            }
            else
            {
        ...
        static public decimal CalculateQuote(string[] details)
        {
            ...
            else
            {
                ...
                over25s++;                              // increment the number of over25 customers
            ...
            if (noClaims > 0)                           // if customer has noClaims bonus,apply a discount
            {
                ...
                {
                    discount = discount + (discount * premium); 
                }//END: (for(noCalims > 0)
            }

            if (age >= 18 && age <= 25)                 // if customer older that 18 calc. quote
            {
                under25s++;                             // increment the number of u25 customers
                quote += (value * premium) - discount;
                quotesUnder25 += quote;
            }
            else
            {
                over25s++;                             // increment the number of u25 customers
                quote += (value* premium) - discount;
                quotesUnder25 += quote;
            }
